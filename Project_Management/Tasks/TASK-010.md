...
### 1. 檔案日誌功能
- [ ] Odds Collector 必須寫入日誌到 `/app/scrapers/logs/odds_YYYY-MM-DD.log`
- [ ] 日誌應該包含爬蟲運行的關鍵資訊
- [ ] Volume 必須正確掛載

### 2. 日誌下載功能
- [ ] Web API 提供 `/api/logs/list` 端點
- [ ] Web API 提供 `/api/logs/download/:source/:filename` 端點
- [ ] 前端 `logs.html` 頁面正常顯示
- [ ] URL: `https://horse.fatlung.com/logs.html`

### 3. 功能驗證（必須全部通過）

#### A. 完整排位表（排位表）
- [ ] API `/api/racecards?date=YYYY-MM-DD` 返回完整排位表
- [ ] 每匹馬有 `jersey_url`（彩衣）
- [ ] 每匹馬有 `horse_name`、`jockey_name`、`draw` 等欄位
- [ ] **驗證命令**：
  ```bash
...
#### B. 正確預測賽果
- [ ] API `/api/predict` 返回不同分數的預測（不是全部相同）
- [ ] 每匹馬有 `score`、`win_probability`、`predicted_rank`
- [ ] **驗證命令**：
  ```bash
  curl -s "https://horse.fatlung.com/api/predict?race_date=2026-03-29&race_no=1&venue=ST" | jq '.predictions[0].score'
  ```
- [ ] **預期結果**：分數應該不同（例如 8.5, 9.2, 7.8 等）

#### C. 賽事資料存在
- [ ] MongoDB `horses` collection 有資料（> 1000）
- [ ] MongoDB `races` collection 有資料（> 2000）
- [ ] **驗證命令**：
  ```bash
...
### 驗收標準（待完成）
- [ ] `/api/logs/download` 端點正常工作
- [ ] `logs.html` 頁面可以成功下載日誌檔案
- [ ] 下載時返回正確的 MIME type 和檔案內容
